...
install_packages() {
  execute_command "sudo apt-get install -y ruby git python3-pip autotools-dev libtool autoconf libopenblas-dev libasound2-dev libusb-dev libdbus-1-dev libglib2.0-dev libudev-dev libical-dev libreadline-dev python3 libatlas-base-dev libopenjp2-7 libtiff6 libjack0 libjack-dev libasound2-dev fonts-freefont-ttf gcc make build-essential git scons swig libavahi-client3 abcmidi" "check_internet"
}
...
install_rtpmidi_server() {
  ...
  execute_command "sudo wget https://ftp.debian.org/debian/pool/main/f/fmtlib/libfmt7_7.1.3+ds1-5_armhf.deb"
  execute_command "sudo dpkg -i libfmt7_7.1.3+ds1-5_armhf.deb"
  execute_command "rm libfmt7_7.1.3+ds1-5_armhf.deb"
